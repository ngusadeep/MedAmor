...
def test_audit_prompt_builder_includes_timeline_and_guideline_ids() -> None:
    ...
    chunks = [
        GuidelineChunk(chunk_id="dm:protocol:0", audit_type="diabetes_management", text="HbA1c target <7%", source_file="protocol.md"),
    ]
    prompt = builder.build(audit_type="diabetes_management", patient_timeline=timeline, guideline_chunks=chunks)
