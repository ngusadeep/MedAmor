                ...
                if "manifestation" in reaction:
                    for manifestation in reaction["manifestation"]:
                        if "text" in manifestation:
                            ...
                            "coding" in manifestation
                            and len(manifestation["coding"]) > 0
                        ):
                            reactions.append(
                                manifestation["coding"][0].get("display", "")
                            )
    ...
    def _format_narrative(self, include_costs: bool, include_codes: bool) -> str:
        ...
        for event in self.timeline_events:
            ...
            if event["type"] == "encounter":
                ...
                lines.append(
                    f"  [{event_time}] CONDITION: {event['condition_name']}{status_str}"
                )
                if event["severity"]:
                ...
                lines.append(
                    f"    Type: {event['allergy_type']}, Category: {event['category']}"
                )
                ...
                lines.append(
                    f"  [{event_time}] MEDICATION PRESCRIBED: {event['medication']}"
                )
                ...
                lines.append(
                    f"  [{event_time}] OBSERVATION: {event['observation']}{value_str}"
                )
                ...
                lines.append(
                    f"  [{event_time}] DIAGNOSTIC REPORT: {event['report_type']}"
                )
                ...
                if include_costs and event["total"] is not None:
                    ...
                    lines.append(
                        f"    Amount: {event['currency']} {event['total']:.2f}"
                    )

            elif event["type"] == "medication_administration":
                lines.append(
                    f"  [{event_time}] MEDICATION ADMINISTERED: {event['medication']}"
                )
                lines.append(f"    Status: {event['status']}")
                if event["dosage"]:
                ...
                lines.append(
                    f"  [{event_time}] MEDICATION REPORTED: {event['medication']}"
                )
                lines.append(f"    Status: {event['status']}")
                ...
                lines.append(
                    f"    Status: {event['status']}, Intent: {event['intent']}"
                )
                if event["requester"]:
                ...
                lines.append(
                    f"  [{event_time}] FAMILY HISTORY: {event['relationship']}"
                )
                if event["conditions"]:
                ...
                if event["num_series"]:
                    lines.append(
                        f"    Series: {event['num_series']}, Instances: {event['num_instances']}"
                    )

            elif event["type"] == "clinical_impression":
                lines.append(
                    f"  [{event_time}] CLINICAL IMPRESSION: {event['summary']}"
                )
                lines.append(f"    Status: {event['status']}")
                if event["findings"]:
                ...
                lines.append(
                    f"  [{event_time}] COMMUNICATION REQUESTED: {category_str}"
                )
                lines.append(f"    Status: {event['status']}")
                if event["priority"]:
                    lines.append(f"    Priority: {event['priority']}")
                ...
                lines.append(
                    f"  [{event_time}] PROVENANCE: {event['activity'] or 'Record activity'}"
                )
                if event["agents"]:
...
def convert_fhir_bundle_to_timeline(
    bundle_json: str, include_costs: bool = True, include_codes: bool = False
) -> str:
    """
...
def load_and_convert(
    filepath: str, include_costs: bool = True, include_codes: bool = False
) -> str:
    """
